...
module.exports = class ConfManager extends require("node-containerpattern") {
	...
	deleteFile () {
		return !this._filePath ? Promise.resolve() : unlink(this._filePath);
	}

	fileExists () {
		return !this._filePath ? Promise.resolve(false) : fileExists(this._filePath);
	}
	...
	save () {
		...
		return !this._filePath ? Promise.resolve() : mkdirp(dirname(this._filePath)).then(() => {

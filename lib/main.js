...
module.exports = class ConfManager extends require("node-containerpattern") {
	...
	constructor (filePath, spaces, commandlineSeparator) {
		...
		if ("undefined" !== typeof filePath && "string" !== typeof filePath) {
			throw new Error("ConfManager/constructor : the filePath is not a string");
		}
		else if ("undefined" !== typeof spaces && "boolean" !== typeof spaces) {
			throw new Error("ConfManager/constructor : the spaces is not a boolean");
		}
		else if ("undefined" !== typeof commandlineSeparator && "string" !== typeof commandlineSeparator) {
			throw new Error("ConfManager/constructor : the commandlineSeparator is not a string");
		}
		else {
	...
	bindShortcut(key, shortkey) {
		...
		if ("string" !== typeof key) {
			throw new Error("ConfManager/bindShortcut : the key is not a string");
		}
		else if ("string" !== typeof shortkey) {
			throw new Error("ConfManager/bindShortcut : the shortkey is not a string");
		}
		else {
			...
			if ("" === key) {
				throw new Error("ConfManager/bindShortcut : empty key");
			}
			else if ("" === shortkey) {
				throw new Error("ConfManager/bindShortcut : empty shortkey");
			}
			else {
	...
	load () {
		...
		return new Promise(function(resolve, reject) {
			...
			try {
				...
				reject("ConfManager/load : Impossible to load the configuration file's data : " + ((e.message) ? e.message : e) + ".");
			}
		...
		_savingPromise = new Promise(function(resolve, reject) {
			...
			try {
				...
				_mkdirp(dirname, function(err) {
					...
					if (err) {
						_savingPromise = null;
						reject("ConfManager/save : Impossible to save the configuration file : " + ((err.message) ? err.message : err) + ".");
					}
					else {
						...
						fs.writeFile(_filePath, JSON.stringify(objects, null, (_spaces) ? "  " : ""), "utf8", function(err) {
							...
							if (err) {
								reject("ConfManager/save : Impossible to save the configuration file's data : " + ((err.message) ? err.message : err) + ".");
							}
							else {
								resolve();
							}
				...
				reject("ConfManager/save : Impossible to save the configuration file's data : " + ((e.message) ? e.message : e) + ".");
				_savingPromise = null;
			}
